...
export class UsuarioAdminEditRoutedComponent implements OnInit {
  ...
  updateForm() {
    ...
    console.log('Tipo de usuario:', this.oUsuario?.tipousuario?.descripcion); 
  ...
  hideModal = () => {
    this.myModal.hide();
    this.oRouter.navigate(['/usuario/view/' + this.oUsuario?.id]);
  }
  ...
  onSubmit() {
    ...
    if (this.oUsuarioForm?.invalid) {
      ...
      this.oUsuarioService.update(formValue).subscribe({
        next: (oUsuario: IUsuario) => {
          this.oUsuario = oUsuario;
          this.showModal('Usuario ' + this.oUsuario.nombre + ' actualizado');
        },
